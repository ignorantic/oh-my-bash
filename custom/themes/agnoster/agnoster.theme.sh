...
# Updated & fixed by Andrii Sorokin ansolaris@gmail.org 3/21/20
...
prompt_git() {
    local ref dirty cached modified untracked deleted stash statuses git
    if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
        if $(inside_gitdir); then
            prompt_segment red white
            PR="${PR}GIT DIR "
        else
            ZSH_THEME_GIT_PROMPT_DIRTY='±'
            dirty=$(git_status_dirty)
            cached=$(git_status_cached)
            modified=$(git_status_modified)
            untracked=$(git_status_untracked)
            deleted=$(git_status_deleted)
            stash=$(git_status_stash)
            ...
            if [[ -n ${dirty} ]] ||
               [[ -n ${stash} ]]; then
                prompt_segment gray default
            fi

            statuses="$cached$modified$untracked$deleted$stash"
            git=${statuses%%*( )}
            PR="$PR$git"
...
git_status_cached() {
   cached=$(git diff --name-only --cached | wc -l)
   [[ ${cached} -gt 0 ]] && echo "📎 $cached "
}

git_status_modified() {
   modified=$(git ls-files --modified | wc -l)
   [[ ${modified} -gt 0 ]] && echo "🎨 $modified "
}

git_status_untracked() {
   untracked=$(git status --porcelain -b 2> /dev/null | grep '??' | wc -l)
   [[ ${untracked} -gt 0 ]] && echo "🌱 $untracked "
}

git_status_deleted() {
   deleted=$(git ls-files --deleted | wc -l)
   [[ ${deleted} -gt 0 ]] && echo "🔥 $deleted "
}

git_status_stash() {
   stash=$(git stash list | wc -l)
   [[ ${stash} -gt 0 ]] && echo "👛 $stash "
}
